...
Next.js and React with TypeScript will use \href{https://tanstack.com/query/latest/docs/framework/react/overview}{TanStack Query} for data fetching and job polling \cite{nextjs}. FastAPI and Pydantic will validate the JSON contracts \cite{fastapi}. SQLAlchemy and Alembic will manage database access and migrations. A Python worker will collect data, fit models and run scenarios.
...
HTTPX collectors will fetch incremental pages. Polars/PyArrow will preserve normalized Parquet evidence. Checkpoints, retries and unique source keys will support safe replay. Fills, transfers, splits, merges and redemptions will be reconciled, with sampled Polygon receipt checks.
...
\midrule
Volatility / asset-date & Past 20-day mean $r^2$, HAR daily/weekly/monthly components, raw event probability, adjusted minus raw probability ($p_{\mathrm{adj}} - p_{\mathrm{raw}}$), probability jump ($\Delta p_{\mathrm{adj}}$), VIX/MOVE baselines & Future $RV_{t,H}$ for $H=1,5,20$ \\
Probability / event-date & Raw probability, spread, anonymous directional flow, eligible-wallet weighted flow, fresh-wallet anomaly flow & Resolved YES/NO label, then $p_{\mathrm{adj}}$ \\
Clustering / wallet-date & Turnover/capital, net YES--NO exposure, event concentration, typical holding duration & Behavior group, with no outcome label \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{figure}[H]
\centering
\input{figures/model-illustrations}
...
\begin{table}[H]
\smalltable
\caption{Candidate implementations in \href{https://scikit-learn.org/stable/supervised_learning.html}{scikit-learn}, \href{https://www.statsmodels.org/stable/index.html}{statsmodels}/\href{https://arch.readthedocs.io/en/latest/}{arch}, and \href{https://xgboost.readthedocs.io/en/stable/}{XGBoost}. The team will explore all techniques and retain models supported by validation.}
...
HAR-RV \& GARCH-X & OLS / GARCH(1,1) with Volatility Process & Multi-frequency HAR components and conditional variance modeling augmented with exogenous prediction-market signals ($p_{\mathrm{adj}}$, $\Delta p_{\mathrm{adj}}$). \\
Logistic regression & LogisticRegression / probability & Regularization strength and wallet-feature contribution. \\
Random Forest \& XGBoost & RandomForestRegressor / XGBRegressor & Non-linear feature interactions across probability spreads, liquidity controls, and macro regime indicators using QLIKE/MSE loss objectives. \\
...
\end{figure}
\textbf{Experiment controls.} The pipeline will fit scaling and imputation procedures on the training folds only. Chronological, event-grouped validation will purge overlapping targets, including for GARCH-X, Random Forest, XGBoost, and MLP tuning. Scores, clusters and probability fits use past data only. Model evaluation for volatility forecasts will report standard robust loss functions (e.g., QLIKE, MSE) alongside directional accuracy. The regression baseline stays fixed. Clusters do not establish skill or ownership. Incomplete positions exclude affected features. Insufficient scenario evidence triggers an unavailable result or a labeled, reviewed stress assumption.

\clearpage
\subsubsection{Research assistant integration}
\href{https://docs.crewai.com/en/concepts/flows}{CrewAI Flows} is the proposed orchestrator. A typed Pydantic state will hold session\_id, event\_id, assets, horizon, as\_of, evidence IDs and run IDs. Three agent roles will work inside an explicit retrieve--calculate--explain flow. The workflow graph controls execution. Event--asset mappings will initially use relational joins, with knowledge-graph retrieval reserved for later exploration.
...
\midrule
RAG + embeddings & Archive event rules, official releases and reviewed mappings. Split the documents into chunks of approximately 180 model tokens, with an overlap of 30 tokens. The Sentence Transformers model \href{https://huggingface.co/sentence-transformers/all-MiniLM-L6-v2}{\texttt{all-MiniLM-L6-v2}} produces 384-dimensional embeddings. Store source, date and embedding version with every chunk. \\
Retrieval & \href{https://github.com/pgvector/pgvector}{pgvector} cosine search plus PostgreSQL keyword search. Merge ranked matches and return an initial set of five results. Filter by event, user access and information cut-off before ranking. Exact prices and positions come from SQL tools. \\
