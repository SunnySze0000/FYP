...
An informative "smart wallet" is an observed account whose previous decisions contain information beyond the market price. The first score will examine whether the wallet repeatedly chose the eventual outcome at informative prices, whether prices subsequently moved in its direction, and how broad its prior history was. Note that a wallet may be informative for one event but not another, and that a wallet's informativeness may change over time. The score will therefore be dated and event-specific, with a separate eligibility test for each forecast cut-off.

Repeated fills will be consolidated into wallet--event decisions. Balanced YES and NO holdings will be distinguished from directional exposure. Scores based on few independent events will be pulled towards the population average, a technique called shrinkage. A provisional eligibility rule is ten earlier resolved events across at least three announcement dates, with five- and twenty-event thresholds checked in sensitivity analysis. If history cannot support this rule, the wallet signal will be marked unavailable rather than assigned an unsupported confidence score.
...
The raw event probability will begin with the YES bid/ask midpoint, recording spread, depth and timestamp. When both outcome books are usable, their midpoints can be normalized for consistency. A missing or one-sided book will be flagged rather than silently replaced by an unrelated latest trade.

A regularized logistic model will calibrate the raw estimate using earlier resolved events and add a small number of wallet signals: overall directional flow, the flow of historically informative wallets and market-quality controls. Coefficients will be fitted on training data rather than chosen to produce a desired wallet-informed probability. Beta calibration is a possible robustness check if justified by the sample \cite{kull}. The raw and wallet-informed probabilities will remain separately visible so the wallet contribution can be evaluated directly.
...
\textbf{Related-event extension.}
Different Polymarket contracts may contain overlapping information because they refer to the same entity, topic, time period or underlying outcome. Subject to data availability, EventLens will test whether features from reviewed related events provide incremental predictive value beyond the selected target event. This related-event analysis is an optional extension. Candidate relationships may be proposed from timestamped contract metadata by the research agent, but will be retained only after deterministic or manual review. The study will interpret these relationships as predictive associations and will not claim causality without a separate causal identification design.

\textbf{Agent ablation.}
The contribution of the research agent will be evaluated by comparing three configurations: fixed rule-based event selection, agent-proposed event selection and a hybrid configuration in which agent proposals undergo deterministic or manual review. All configurations will use the same point-in-time data, forecast dates and downstream statistical model. The LLM will propose and explain relationships; numerical forecasts will continue to come from reproducible calculation tools.
...
\end{equation}
This daily-data proxy for realized variance measures the size of price movements without treating gains and losses as cancellations, but is noisy, especially for one day. Comparisons use the same horizon and units. Displayed volatility is the square root of the corresponding variance estimate.
...
\textbf{Augmented model.} The same regression retains that input and adds the market-implied probability $p_{\text{raw}}$ and wallet-informed probability difference $p_{\text{adj}}-p_{\text{raw}}$. Both versions use identical forecast dates, targets and training windows restricted to dates with usable event evidence. The augmented model therefore tests the combined contribution of prediction-market information, while separate probability experiments test whether wallet histories themselves add information.
