...
We will backfill only eligible events and relevant wallet histories, using indexed data and hosted RPC without running a blockchain node. Goldsky warns that legacy subgraphs may be incomplete after the 2026 contract migration; the audit will verify version-specific coverage \cite{goldsky}.

The audit will test sample downloads and fix provider, adjustment convention and coverage for each experiment. Forward market snapshots will be collected from project start and equity bars refreshed after each session. Permitted university data are a fallback. Access and historical depth remain unconfirmed; options quotes are optional.
