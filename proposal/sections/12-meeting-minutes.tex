...
2.1 All members had reviewed the FYP proposal template and familiarized themselves with the submission requirements. \\
...
3.1 The group agreed on a structured timeline before the proposal submission: idea-sharing on 16 August, buffer sessions on 22 and 29 August, internal deadline (draft to communication tutor) on 3 September, review of tutor comments on 6 September, soft submission deadline on 10 September and hard submission deadline on 11 September. \\
...
\textbf{3. Discussion items} \\
3.1 The team reviewed the proposed user workflow: a user selects a supported market category (e.g., a Federal Reserve rate-cut event), the system applies smart-wallet identification to derive a wallet-informed probability, and an optional simple backtest of a volatility relative-value strategy is run on the result. \\
3.2 The team discussed the intended MVP dashboard: user-selected event categories, intermediate outputs (smart wallets, wallet-informed probability) and final outputs (volatility and risk metrics such as value at risk (VaR)). \\
3.3 The team clarified the role of the LLM agent: summarizing smart-wallet evidence for the user, mapping a user's watchlist or portfolio to relevant Polymarket events, and offering context-appropriate suggestions once the user has entered their holdings. \\
...
3.5 Referring to the FYP grading rubric (approximately 60\% objectives/methodology/background, 30\% organization/clarity/presentation, 10\% planning and timeline), the team agreed the draft needed significant shortening, targeting roughly 10--15 pages, with the Design and Application subsection cut most aggressively. \\
...
\textbf{3. Discussion items} \\
3.1 The team revisited smart-wallet identification in more depth: given the limited history available on Polymarket (approximately three years) and the absence of a labeled dataset, the team agreed to first define rule-based labels (e.g., a historical win-rate threshold) and then train a model on these labels so that new, previously unseen wallets can also be scored. \\
3.2 The team discussed mapping the wallet-informed probability to asset volatility, identifying GARCH-family models as a candidate approach for incorporating the wallet-informed probability as an input feature, while noting this mapping step has fewer observations per event than the wallet-scoring step and needs a simpler, more robust design. \\
...
3.4 The team clarified that the machine-learning components (wallet scoring, probability-to-volatility mapping) and the LLM research agent (event--asset mapping and user-facing explanation) are complementary rather than overlapping, so the design satisfies the project's AI requirement without redundancy. \\
3.5 The team discussed two possible uses for the volatility output -- portfolio risk management versus an actively traded volatility strategy -- and agreed to describe both directions in the proposal while prioritizing the risk-management framing, since the primary target user is a retail investor rather than a derivatives trader. \\
3.6 The team agreed to source on-chain settlement data via Polygon RPC, in addition to Polymarket's own API for event metadata, to keep the project's Web3 focus. Initial event coverage will prioritize finance/macroeconomics and politics/geopolitics categories over sports or other categories. \\
...
3.8 The team noted an open question requiring further work: how to weight and aggregate multiple related Polymarket bets into a single wallet-informed volatility estimate for a given asset.
...
\clearpage
\subsection{Minutes of the Supervisor Consultation}

\textbf{Date:} 10 September 2026 (Thursday) \\
\textbf{Time:} 10:00 \\
\textbf{Place:} Online meeting \\
\textbf{Present:} Prof.\ Shuai Wang and the project team \\
\textbf{Recorder:} Team representative

\textbf{1. Review of the proposal} \\
1.1 The supervisor was satisfied with the team's progress and considered the proposal report suitable for submission after final refinement. \\
1.2 The supervisor did not confirm a preference for passive voice. The team therefore agreed to prioritize clear and consistent academic writing. \\

\textbf{2. Suggestions} \\
2.1 The supervisor suggested investigating relationships among Polymarket events. Events that are not directly selected for analysis may still contain related information through shared topics, entities or outcomes. \\
2.2 The supervisor suggested an ablation or comparison study between fixed rule-based signals and analysis assisted by an LLM agent, with a hybrid approach as an additional configuration if capacity permits. \\
2.3 The team noted that predictive relationships may decay over time and agreed that any proposed signal should be assessed across chronological and rolling evaluation periods. \\
2.4 The supervisor clarified that the cross-event and agent-ablation studies were suggestions rather than required additions and could be prioritized according to available time and capacity. \\

\textbf{3. Resources} \\
3.1 The team asked about funding for external APIs and related services. The supervisor indicated that a total budget in the range of 1,000--2,000 would be reasonable, subject to confirmation, with a possible limited top-up if justified later. \\

\textbf{4. Actions} \\
4.1 The team will refine the proposal to record related-event analysis and agent ablation as bounded extensions rather than unconditional MVP commitments. \\
4.2 The team will confirm the approval process before incurring external-service costs. \\
4.3 The team will complete language and consistency checks before submission.
